...
\usepackage{parskip} % for noindent global
...
\AtBeginDocument{
  \renewcommand{\minus}{%
    \mathbin{%
      \mathchoice
        {\scalebox{0.8}{$\text{\textbf{-}}$}} % Display and Text style
        {\scalebox{0.8}{$\text{\textbf{-}}$}} % Script style
        {\scalebox{0.5}{$\text{\textbf{-}}$}} % Script-script style
        {\scalebox{0.8}{$\text{\textbf{-}}$}} % Default fallback
    }%
  }
}
  ...
  \IfBooleanTF{#1}{%
    ...
    \IfValueTF{#2}{
      \errmessage{magnitude: Star mode. Don't specify the [size].}
        ...
        \left\lvert \hspaceNorm #3 \hspaceNorm \right\rvert
    }
...
\NewDocumentCommand{\floor}{s o m}{%
  \IfBooleanTF{#1}{%
    % Star version: auto-sizing with \left\right
    \IfValueTF{#2}{
      \errmessage{floor: Star mode. Don't specify the [size].}
    }{
      \lfloor \hspaceNorm #3 \hspaceNorm \rfloor
    }
  % Non-star version: manual sizing
  }{
    \IfValueTF{#2}{% if [size] 
      #2\lfloor \hspaceNorm #3 \hspaceNorm #2\rfloor
    }{% elif no [size]
      \errmessage{floor: Size command required.}
    }
  }
}
